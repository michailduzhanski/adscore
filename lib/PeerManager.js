...
PeerManager.prototype.start = function() {
  ...
  if (!this.timer) {
	...
	try {
		...
		log.err('trouble in timer', e);
	  }
    
  }else{
	log.info("timer is not empty: ") 
	log.info(this.timer) 
  }
};
...
PeerManager.prototype.addPeer = function(peer, port) {
  log.info("PeerManager.prototype.addPeer: "+peer)
  ...
  log.info("PeerManager.prototype.addPeer peers size: "+this.peers.length)
};

PeerManager.prototype.removePeer = function(peer) {
	log.info("PeerManager.prototype.removePeer")
...
PeerManager.prototype.checkStatus = function checkStatus() {
  ...
  log.info("PeerManager.prototype.checkStatus start peers: " + this.peers.length)
  if (this.peers.length) {
	...
	log.info(this.connections.length)
...
PeerManager.prototype.connectTo = function(peer) {
  log.info('PeerManager connecting to ' + peer);
  try {
...
PeerManager.prototype.handleVersion = function(e) {
  ...
  if (this.peerDiscovery &&
    (e.message.version >= 31402 || this.peers.length < this.maxConnections)) {
    e.conn.sendGetAddr();
    e.conn.getaddr = true;
  }
};
...
PeerManager.prototype.handleAddr = function(e) {
	log.info("PeerManager.prototype.handleAddr start")
  ...
  if (e.message.addrs.length < this.minKnownPeers) {
...
PeerManager.prototype.handleError = function(e) {
  ...
  log.info('unkown error with peer ' + e.peer + ' (disconnecting): ' + e.err);
...
PeerManager.prototype.handleDisconnect = function(e) {
  ...
  log.info("after remove peer pools size: "+this.pool.length)
  if (this.pool.length) {
  ...
  if (!this.connections.length) {
    ...
    this.isConnected = false;
	  ...
	  log.info("connections: "+this.connections.length)
  }
};
...
PeerManager.prototype.discover = function(options, callback) {
  ...
  self.limit = options.limit || this.maxConnections;
